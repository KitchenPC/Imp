...
namespace Imp
{
    ...
    public class ProxyRendering<T> : IProxyRendering
    {
        ...
        private static readonly Regex routeParser = new Regex(@"\{([a-zA-Z\d-]+)}", RegexOptions.Compiled);
        ...
        private string getProxyCall(MethodInfo methodInfo)
        {
            ...
            var route = buildRoute(methodName, methodInfo.GetCustomAttribute<RouteAttribute>()?.Template);

            var inboundParameters = methodInfo.GetParameters()
                .Select(p => p.Name)
                .ToList()
            ...
            {
                // Build payload
                var payload = String.Join(", ", methodInfo.GetParameters()
        ...
        public async Task RenderProxy(HttpResponse response)
        {
            ...
            if (stream != null)
            {
                ...
                {
                    string script = new String(new UTF8Encoding().GetChars(bytes))
                        .Replace("/*[PROXY_FUNCTIONS]*/", getFunctions());
